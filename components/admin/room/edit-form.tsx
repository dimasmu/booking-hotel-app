    ...
    const handleUpload = () => {
        ...
        startTransition(async () => {
            try {
                ...
                if (!img.url) {
                    console.log("Error: Empty URL received");
                    setMessage("Error: Empty URL received");
                    return;
                }
                setImage(img.url);
            } catch (error) {
        ...
        <form action={formAction}>
            <div className="grid md:grid-cols-12 gap-5">
                <div className="col-span-8 bg-white p-4">
                    ...
                    <div className="mb-4">
                        <textarea name="description" rows={8} placeholder="Description" className="py-2 px-4 rounded-sm border border-gray-400 w-full"></textarea>
                        <div aria-live="polite" aria-atomic="true">
                    ...
                    <div className="mb-4 grid md:grid-cols-3">
                        {amenities.map((item) => (
                            <div className="flex items-center mb-4" key={item.id}>
                                <input type="checkbox" name="amenities" className="w-4 h-4 text-blue-600 bg-gray-100 border border-gray-300 rounded" defaultValue={item.id} />
                ...
                <div className="col-span-4 bg-white p-4">
                    <label htmlFor="input-file" className="flex flex-col mb-4 items-center justify-center aspect-video border-2 border-gray-300 border-dashed rounded-md cursor-pointer bg-gray-50 relative">
                        ...
                        {!image || image.trim() === "" ? (
                            ...
                            <>
                                {console.log("Rendering Image with src:", image)}
                                <Image
                                    src={image}
                                    alt="image"
                                    width={640}
